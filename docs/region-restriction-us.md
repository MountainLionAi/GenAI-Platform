...
| `in_user_wl` | 当前用户 ID 是否命中白名单（env 或 Redis 任一即 `true`） |
| `in_ip_wl` | 当前客户端 IP 是否命中白名单（env 或 Redis 任一即 `true`） |
...
`env` 来自 `REGION_WHITELIST_USER_IDS`、`REGION_WHITELIST_IPS`；`redis` 来自对应 Redis Set。两侧数组均可为 `[]`。中间件放行时仍对两侧做**并集**判定。
