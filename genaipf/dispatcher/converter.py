    ...
    if func_name in preset_entry_mapping:
        ...
        if preset_conf.get("has_preset_content") and (_param.get("need_chart") or preset_conf.get("need_preset")) and presetContent != {}:
            ...
            if func_name in preset_entry_top_mapping:
                ...
                _data = {}
    if (func_name not in not_need_search and sub_func_name not in ['coin_swap1','transfer_only', 'why_can_not_transfer_out', 'buy_but_not_receive']) or (func_name == 'generate_report' and presetContent == {}) or source in ['v201', 'v202', 'v203', 'v210'] or func_name in need_gpt or (owner == 'Trust Wallet' and func_name == 'tx_hash_analysis'):
        _messages = [x for x in messages if x["role"] != "system"]
        msgs = _messages[::]
