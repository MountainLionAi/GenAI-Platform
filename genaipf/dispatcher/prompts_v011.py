...
class LionPrompt:
    ...
    def get_afunc_prompt(cls, language=_default_lang, picked_content="", related_qa=[], model='', data={}):
        return _get_default_afunc_prompt(language, picked_content, related_qa, model, data)

    @classmethod
    def get_aref_answer_prompt(cls, language=_default_lang, preset_name=None, picked_content="", related_qa=[], model='', data={}):
        return _get_default_aref_answer_prompt(language, picked_content, related_qa, model, data)
...
if PLUGIN_NAME:
    plugin_submodule_name = f'{PLUGIN_NAME}.dispatcher.prompts_v011'
