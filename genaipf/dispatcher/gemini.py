...
async def async_get_mime_files_cache(b64s_and_fname_l):
    ...
    if is_in:
        ...
        cache = await async_caching_CachedContent_create(
            model='models/gemini-3.5-flash-001',
...
async def async_get_gemini_chat_stream(gemini_contents, system_message):
    model = genai.GenerativeModel(model_name='gemini-3.5-flash', system_instruction=system_message)
...
model = genai.GenerativeModel(model_name='gemini-3.5-flash', system_instruction=None)
...
model = genai.GenerativeModel(model_name='gemini-3.5-flash', system_instruction="you can say up to 10 words")
