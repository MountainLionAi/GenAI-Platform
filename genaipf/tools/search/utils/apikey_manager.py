...
# 根据接入的sdk类型获取api_key
def get_api_key_by_type(client_type='exa'):
    redis_client = RedisConnectionPool().get_connection()
    redis_key = REDIS_KEYS['RAG_API_KEYS']['KEYS_STATUS'].format(client_type)
    random_key = ''
    avaiable_keys = []
    try:
        ...
        logger.error(f'获取{client_type}--apikey错误: {e}')
    return random_key


# 向redis集合中添加key
def set_api_key_by_type(keys, client_type='exa'):
    redis_client = RedisConnectionPool().get_connection()
    redis_key = REDIS_KEYS['RAG_API_KEYS']['KEYS_STATUS'].format(client_type)
...
# 设置某一个apikey不可用
def set_api_key_unavaiable(key, client_type='exa'):
    redis_client = RedisConnectionPool().get_connection()
    redis_key = REDIS_KEYS['RAG_API_KEYS']['KEYS_STATUS'].format(client_type)
