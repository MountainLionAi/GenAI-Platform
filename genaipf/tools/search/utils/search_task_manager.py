...
from .smart_category_matcher import get_category_matcher
...
async def aload_web(url):
    try:
        ...
        llm = ChatOpenAI(temperature=0, openai_api_key=openai_api_key, model_name="gpt-5.4-mini")
...
async def check_ai_ranking(messages, language, source=''):
    ...
    try:
        ...
        logger.info(f'分析用户消息: {user_content}')
        ...
        # 检查是否包含排序/对比关键词
        ranking_keywords = [
            '最', '最好', '最差', '排名', '排行', '榜单', 'top', 'top10', '对比', '比较', 
            '哪个好', '哪个更', '推荐', '评测', '清单', '汇总', 'best', 'worst', 'ranking',
            'compare', 'versus', 'which is better', 'recommend', 'review', 'list'
        ]
        
        has_ranking_intent = any(keyword in user_content.lower() for keyword in ranking_keywords)
        
        if has_ranking_intent:
            # 使用智能匹配器快速匹配项目类型
            matched_categories = category_matcher.match_categories(user_content, max_matches=3)
            
            if matched_categories:
                # 选择置信度最高的匹配
                best_match = matched_categories[0]
                if best_match[1] > 0.5:  # 置信度阈值
                    ranking_data['need_ranking'] = True
                    ranking_data['category'] = best_match[0]
                    ranking_data['keywords'] = category_matcher.extract_keywords_from_text(user_content)
                    
                    # 确定排序维度
                    ranking_type = _determine_ranking_type(user_content)
                    ranking_data['ranking_type'] = ranking_type
                    
                    logger.info(f'智能匹配结果: {best_match[0]} (置信度: {best_match[1]:.2f})')
                    return ranking_data
            
            # 如果智能匹配失败，回退到LLM分析
            logger.info('智能匹配未找到合适分类，回退到LLM分析')
            return await _fallback_llm_analysis(messages, language, source)
        else:
            # 检查其他类型的需求
            return await _check_other_ranking_types(messages, language, source)
...
def _determine_ranking_type(user_content: str) -> str:
    """确定排序维度"""
    content_lower = user_content.lower()
    
    # 人气/热度相关
    if any(keyword in content_lower for keyword in ['人气', '热度', '活跃', '增长', 'tvl', '交易量', 'popularity', 'adoption']):
        return 'popularity'
    
    # 安全相关
    if any(keyword in content_lower for keyword in ['安全', '风控', '合规', 'security', 'risk', 'compliance']):
        return 'security'
    
    # 性能相关
    if any(keyword in content_lower for keyword in ['吞吐', '可扩展性', '稳定性', 'performance', 'scalability', 'stability']):
        return 'performance'
    
    # 成本相关
    if any(keyword in content_lower for keyword in ['手续费', '成本', '性价比', 'fees', 'cost', 'cost-efficiency']):
        return 'cost'
    
    # 速度相关
    if any(keyword in content_lower for keyword in ['速度', '确认时间', '延迟', 'speed', 'latency', 'confirmation']):
        return 'speed'
    
    return 'popularity'  # 默认按人气排序


async def _fallback_llm_analysis(messages, language, source):
    """回退到LLM分析的fallback方法"""
    try:
        ...
        logger.info(f'LLM分析结果: {ranking_result}')
        
        if ranking_result and ranking_result.get('need_ranking'):
...
async def _check_other_ranking_types(messages, language, source):
    """检查其他类型的排名需求"""
    try:
        _messages = {"messages": [messages['messages'][-1]]}
        msgs = LionPromptCommon.get_prompted_messages("check_ai_ranking", _messages, language)
        ...
        # 检查各种排名类型
        if ranking_result.get('need_ranking') and ranking_result.get('category') and ranking_result['category'] != 'null':
            return ranking_result
        elif ranking_result.get('need_person_ranking') and ranking_result.get('person_ranking_type') and ranking_result.get('target_entity'):
            return ranking_result
        elif ranking_result.get('need_project_research') and ranking_result.get('project_keywords') and len(ranking_result['project_keywords']) != 0:
            return ranking_result
        elif ranking_result.get('need_investment_ranking') and ranking_result.get('investment_ranking_type'):
            return ranking_result
        else:
            return {
                "need_ranking": False,
