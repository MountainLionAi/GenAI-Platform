...
async def add_feedback(request: Request):
    ...
    if not request_params or not request_params['seriousness'] or not request_params['type'] or not request_params['content'] or not request_params['contact']:
    ...
    info = [seriousness, type, bug_location, content, base64_content, userid, contact]
    try: 
