...
llm_config = {
    "config_list": [{"model": "gpt-5.2", "api_key": openai_api_key}],
    "cache_seed": "abc123",
    "temperature": 0.1,
    "timeout": 5 * 60,
}
...
class Saler:
    def __init__(self, pack_id, deliver_id):
        self.pack_id = pack_id
        self.deliver_id = deliver_id
    def pack(self):
        return f"pack_id: {self.pack_id}"
    def deliver(self):
        return f"deliver_id: {self.deliver_id}"
...
async def deliver(
    ...
    await asyncio.sleep(1)
    return f'''
deliver {goods_name}, {self.ctx.deliver()}
'''
...
from genaipf.agent.autogen import AutoGenMultiAgent

agma = AutoGenMultiAgent(
    llm_config,
    agents_config,
    saler
)
...
async def main():
    x = await agma.a_initiate_chat(f"I want an apple. at now {datetime.now()}")
